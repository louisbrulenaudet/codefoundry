...
| `packages/typescript-config` | `@monorepo/typescript-config` | `config` | tsconfig presets (published) |
| `packages/conventions` | `@monorepo/conventions` | `config` | conventions package: manifest + templates for `@monorepo/cli` |
| `packages/cli` | `@monorepo/cli` | `lib` | the `monorepo` CLI (init / sync / check); moving to its own repo later |
