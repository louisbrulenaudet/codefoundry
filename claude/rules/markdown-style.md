...
Markdown is **not hard-wrapped**. One line per paragraph, one line per list item, one line per table row. Lines of 250-700 characters are correct, not a defect to fix. There is no markdown formatter - `oxfmt` does not touch `.md` - so nothing will reflow this for you and nothing will reflow it back.
...
- Applies to every `.md` and `.mdc` in the repository, including `README.md`, `AGENTS.md`, `CLAUDE.md`, `.changeset/**`, and rule trees.
